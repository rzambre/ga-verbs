 ...
 * The network configuration is the same as in state-of-the-art
 * MPI implementation where each process has its endpoint. In the multi-thread
 * environment this translates to a separate context for each thread.
 * 
 * Only 1 context is needed for Rank 1. In reality, only 1 QP is needed but we create
 * multiple QPs since we are working with RC QPs.
 */
...
struct ibv_context *server_dev_context;
struct ibv_pd **pd;
struct ibv_pd **parent_d;
struct ibv_td **td;
struct ibv_pd *server_pd;
...
int main (int argc, char *argv[])
{
	...
	if (rank) {
		...
		global_a_mr = ibv_reg_mr(server_pd, global_a, global_a_size * sizeof *global_a, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ);
		if (!global_a_mr) {
		...
		global_b_mr = ibv_reg_mr(server_pd, global_b, global_b_size * sizeof *global_b, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ);
		if (!global_b_mr) {
		...
		global_c_mr = ibv_reg_mr(server_pd, global_c, global_c_size * sizeof *global_c, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ);
		if (!global_c_mr) {
		...
		for (tid = 0; tid < num_threads; tid++) {
			...
			tile_a[tid].mr = ibv_reg_mr(pd[tid], tile_a[tid].tile_arr, tile_a_size * sizeof *tile_a[tid].tile_arr, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
			if (!tile_a[tid].mr) {
			...
			tile_b[tid].mr = ibv_reg_mr(pd[tid], tile_b[tid].tile_arr, tile_b_size * sizeof *tile_b[tid].tile_arr, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
			if (!tile_b[tid].mr) {
			...
			tile_c[tid].mr = ibv_reg_mr(pd[tid], tile_c[tid].tile_arr, tile_c_size * sizeof *tile_c[tid].tile_arr, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
			if (!tile_c[tid].mr) {
	...
	if (!rank) {
		...
		#pragma omp parallel private(ret) firstprivate(element_bytes) 
		{
			...
			ret = ibv_query_qp(qp[tid], &attr, IBV_QP_CAP, &qp_init_attr);
			if (ret) {
			...
			my_flow_vars.wc = WC;
			my_flow_vars.my_qp = qp[tid];
			my_flow_vars.my_cq = cq[tid];
...
static int alloc_ep_res(void)
{
	...
	if (!rank) {
		dev_context = malloc(num_threads * (sizeof *dev_context));
		if (!dev_context) {
		...
		pd = malloc(num_threads * (sizeof *pd));
		if (!pd) {
		...
		parent_d = malloc(num_threads * (sizeof *parent_d));
		if (!parent_d) {
		...
		td = malloc(num_threads * (sizeof *td));
		if (!parent_d) {
	...
	cq = malloc(num_threads * (sizeof *cq));
	if (!cq) {
	...
	qp = malloc(num_threads * (sizeof *qp));
	if (!qp) {
...
{
	int ret = 0;
	int dev_i, ctx_i, pd_i, cq_i, qp_i;
	...
	/* Acquire a Device Contexts */	
	if (rank) {
		server_dev_context = ibv_open_device(dev);
		if (!server_dev_context) {
			fprintf(stderr, "Couldn't get server context for %s\n",
			ibv_get_device_name(dev));
			ret = EXIT_FAILURE;
			goto clean_dev_list;
		}
	} else {
		for (ctx_i = 0; ctx_i < num_threads; ctx_i++) {
	...
	/* Open up a Protection Domains */
	if (rank) {
		server_pd = ibv_alloc_pd(server_dev_context);
		if (!server_pd) {
			fprintf(stderr, "Couldn't allocate server PD\n");
			ret = EXIT_FAILURE;
			goto clean_dev_context;
		}
	} else {
		for (pd_i = 0; pd_i < num_threads; pd_i++) {
			ctx_i = pd_i;
		...
		if (dynamic) {
			for (qp_i = 0; qp_i < num_threads; qp_i++) {
				...
				pd_init.td = td[qp_i];
				pd_init.comp_mask = 0;
				parent_d[qp_i] = ibv_alloc_parent_domain(dev_context[qp_i], &pd_init);
				if (!parent_d[qp_i]) {
	...
	for (cq_i = 0; cq_i < num_threads; cq_i++) {
		ctx_i = cq_i;
		if (rank)
			cq[cq_i] = ibv_create_cq(server_dev_context, cq_depth, NULL, NULL, 0);
		else
	...
	for (qp_i = 0; qp_i < num_threads; qp_i++) {
		...
		struct ibv_qp_init_attr qp_init_attr = {
			.send_cq = cq[cq_i],
			.recv_cq = cq[cq_i], // the same CQ for sending and receiving
			.cap     = {
		...
		if (rank)
			qp[qp_i] = ibv_create_qp(server_pd, &qp_init_attr); // this puts the QP in the RESET state
		else {
			if (dynamic)
				qp[qp_i] = ibv_create_qp(parent_d[qp_i], &qp_init_attr); // this puts the QP in the RESET state
			else
				qp[qp_i] = ibv_create_qp(pd[qp_i], &qp_init_attr); // this puts the QP in the RESET state
		}
		if (!qp[qp_i]) {
...
clean_qp:
	for (qp_i = 0; qp_i < num_threads; qp_i++) { ibv_destroy_qp(qp[qp_i]); }
	
clean_cq:
	for (cq_i = 0; cq_i < num_threads; cq_i++) { ibv_destroy_cq(cq[cq_i]); }

clean_pd:
	if (rank)
		ibv_dealloc_pd(server_pd);
	else
		for (pd_i = 0; pd_i < num_threads; pd_i++) { ibv_dealloc_pd(pd[pd_i]); }

clean_dev_context:
	if (rank)
		ibv_close_device(server_dev_context);
	else
		for (ctx_i = 0; ctx_i < num_threads; ctx_i++) { ibv_close_device(dev_context[ctx_i]); }
...
static int connect_eps(void)
{
	...
	int *my_lid = calloc(num_threads, sizeof(int));
	int *dest_lid = calloc(num_threads, sizeof(int));
	int *dest_qp_index = calloc(num_threads, sizeof(int));

	/* Query port to get my LID */
	for (tid = 0; tid < num_threads; tid++) {
		struct ibv_port_attr ib_port_attr;
		if (rank)
			ret = ibv_query_port(server_dev_context, ib_port, &ib_port_attr);
		else
			ret = ibv_query_port(dev_context[tid], ib_port, &ib_port_attr);
		if (ret) {
			fprintf(stderr, "Failed to get port info\n");
			ret = EXIT_FAILURE;
			goto exit;
		}
		my_lid[tid] = ib_port_attr.lid;
	}

	if (!rank) {
		target = 1;
		for (tid = 0; tid < num_threads; tid++) {
			MPI_Send(&my_lid[tid], 1, MPI_INT, target, 0, MPI_COMM_WORLD);
			MPI_Recv(&dest_lid[tid], 1, MPI_INT, target, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int free_ep_res(void)
{
	...
	if (rank) {
		...
		for (tid = 0; tid < num_threads; tid++) {
			...
			for (tid = 0; tid < num_threads; tid++) {
				ibv_dealloc_pd(pd[tid]);
			}
			
			for (tid = 0; tid < num_threads; tid++) {
				ibv_close_device(dev_context[tid]);
			}
		}
	}
...
int read_args(int argc, char *argv[])
{
	...
	while (1) {
		op = getopt_long(argc, argv, "h?d:t:n:c:m:j:k:i:p:q:T:R:xCv", long_options, NULL);
		if (op == -1)
			break;
